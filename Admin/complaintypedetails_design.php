...
<body>
     ...
     <!--2nd column start-->
         ...
         <!--nav bar start-->
            <nav class="navbar navbar-expand-sm bg-dark ">
              <a class="navbar-brand text-white">Complain Type Details</a>
         ...
         <!-- 2nd nav bar start-->
                 ...
                 <nav aria-label="breadcrumb ">
                      ...
                      <ol class="breadcrumb arr-right bg-secondary-50 ">
                        ...
                        <li class="breadcrumb-item "><a href="" class="text-dark">Complain Type</a></li>
        ...
        <!-- 2nd nav bar end-->
         ...
         <div class="card">
              <div class="card-header">
                <strong>ADD COMPLAIN TYPE</strong>
              </div>
            
              <div class="card-body">
                  <div class="col-sm-8">
                      <form action="insertcomplaintype_code.php" method="post">
                      <div class="card-header" style="height:100px">              
                          <div class="form-group">
                            <label for="dept"><strong> COMPLAIN TYPE</strong></label>
                            <input type="text" class="form-control" name="complain" id="complaintype" ><br>
 ...
 <div class="card">
      <div class="card-header">
         <strong> COMPLAIN TYPE DETAILS</strong>
      </div>
      <div class="card-body">
      <form action="alldelet_complain_code.php" method="post">
        <table class="table">
        ...
        <tbody>
        <?php
              $cmd="select * from complaintype_details order by complain_type asc";
              $data=$conn->query($cmd);
              $slno=1;
              foreach($data as $var)
             {
              ?>
                  <tr>
                        <td><input type="checkbox" name="chkUser[]" id="chkUser" value="<?php echo $var['complain_id'] ?>"/></td>
                        <td ><?php echo $slno++; ?></td>
                        <td ><?php echo $var['complain_type']?></td>
                        <td ><?php echo $var['status']?></td>
                        <td >
                            </div>   
                              <a href="edit_complaintype_design.php?id=<?php echo $var['complain_id'] ?>"><i class='fas fa-edit'></i></a>
                              <a href="delete_complaintype_code.php?id=<?php echo $var['complain_id'] ?>"><i class='fas fa-trash'></i></a>
                            </div>
                        </td>
                    <?php  }?> 
                  </tr>
     ...
     $(document).ready(function(){
	     $('#complaintype').change(function(){
		      $.ajax({
					...
					data:{complain: $('#complaintype').val()},
					type:'post',
					success:function(response)`{
						if(response==1)
						 {
							$('#err').text('Complain Type already exists');
						 }
						 else
						 {
							$('#err').text('');
