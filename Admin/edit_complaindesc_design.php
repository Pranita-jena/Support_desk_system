   ...
   <div class="container-fluid bg-light" >
    ...
    <!-- 1st column-->
     ...
     <div class="row">
              <div class="card-body">
                  <div class="col-xs-6">
                  <form action="update_complaindescription_code.php" method="post"> 
                   
                           <?php
								$cmd="select * from complain_description_details where complain_desc_id =".$_GET['id'];
								$datadesc=$conn->query($cmd);
                      ...
                      <div class="card-header">              
                          <div class="form-group">
                            <label for="dept"><strong>COMPLAIN TYPE</strong></label>
                                           <input type="hidden" name="id" value="<?php echo $row['complain_desc_id']?>" /> 
                                           <select class="form-control" name="complaintype" required>
                                           
                                                                        <option value="">Select</option>
                                                                        <?php
                                                                            $cmd="select complain_id,complain_type from complaintype_details";
                                                                            $data=$conn->query($cmd);
                                                                            foreach($data as $var)
                                                                            {
                                                                        ?>
                                                                            <option <?php echo($row['complain_id'] == $var['complain_id'] ? 'selected' : '') ?> value="                                                                                    <?php echo $var['complain_id']; ?>"><?php echo $var['complain_type']; ?>
                                                                            </option>
                                                                       <?php
                                                                            }
                                                                        ?>
                      ...
                      <div class="card-header">              
                          <div class="form-group">
                            <label for="dept"><strong>COMPLAIN DESCRIPTION</strong></label><br>
                                ...
                                <input type="text" class="form-control" name="cdesc"  id="complain" value="<?php echo $row['complain_desc'] ?>" >
